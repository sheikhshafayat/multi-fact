...
class DocDB(object):
    ...
    def __init__(self, db_path=None, data_path=None):
        ...
        if len(cursor.fetchall())==0:
            print("here")
            # usually this line is not executed 
...
class Retrieval(object):
    ...
    def get_bm25_passages(self, topic, query, passages, k):
        ...
        return [passages[i] for i in indices], indices
    ...
    def get_passages(self, topic, question, k):
        ...
        if (cache_key not in self.cache) or (indices_key not in self.cache):
            ...
            if self.retrieval_type=="bm25":
                self.cache[cache_key], indices = self.get_bm25_passages(topic, retrieval_query, passages, k)
            else:
                self.cache[cache_key], indices = self.get_gtr_passages(topic, retrieval_query, passages, k)
            assert len(self.cache[cache_key]) in [k, len(passages)]
            
            indices = indices.tolist()
            self.cache[indices_key] = indices
